...
class FlowNetgp(FlowNetC):

    def __init__(self, normalization='Example', batchNorm=False, div_flow=1):
        super(FlowNetgp, self).__init__(batchNorm=batchNorm, div_flow=1)
        self.normalization = normalization
        self.c = 256
        self.conv_a=conv(self.batchNorm, 256,  self.c, kernel_size=1, stride=1)
        self.conv_b=conv(self.batchNorm, 256,  self.c, kernel_size=1, stride=1)
        self.conv_corr   = conv(self.batchNorm, 441,  self.c, kernel_size=1, stride=1)
        self.softmax = nn.Softmax(dim=2)
        self.fusion = conv(self.batchNorm, self.c,  441, kernel_size=1, stride=1)
    ...
    def forward(self, inputs):
        ...
        residual = out_corr
        va = self.conv_a(out_conv3a)
        vb = self.conv_b(out_conv3a)
        #IPython.embed()
        bs = va.size(0)
        c = va.size(1)
        h = va.size(2)
        vab = torch.bmm(va.view([bs, c, -1]).transpose(2, 1), vb.view([bs, c, -1]))
        scale_factor = (1.0 / math.sqrt(float(vab.size(0))))
        #IPython.embed()
        vab_scale = vab * scale_factor
        vab_s = self.softmax(vab_scale)
        #vab_s = self.dropout(vab_s)
        
        corr = self.conv_corr(out_corr)
        corr2 = corr.view([bs,c,-1]).transpose(2,1)
        corr3 = torch.bmm(vab_s, corr2)
        corr4 = corr3.transpose(2,1).contiguous().view([bs,c,h,-1])
        corr5 = self.fusion(corr4)
        out_corr = residual + corr5
